...
//		Microsoft.Win32.RegistryKey         gRegKey=Microsoft.Win32.Registry.CurrentUser.CreateSubKey(@"Software\kenjisoft\UyghurEditPP");
		...
		void menuSozImla(object sender, System.Windows.RoutedEventArgs e)
		{
			...
			if(menuNamzat == gMenuSozToghra){
				try{
					soz = Uyghur.ToUEY(soz);
					if(soz ==null){
						soz = (string)menuNamzat.Tag;
					}
					File.AppendAllText(gImlaIshletkuchi, soz + " 1"+System.Environment.NewLine,System.Text.Encoding.UTF8);
				}catch{
...
//			}
		...
		void namzat_Click(object sender, System.Windows.RoutedEventArgs e)
		{
			...
			try{
				...
				File.AppendAllText(gImlaXataToghra,tx+System.Environment.NewLine,System.Text.Encoding.UTF8);
			}catch{
...
//			if(gConfig.Contains("RTL")){
//				gRTL=(bool)gConfig["RTL"];
//			}
//			else{
//				gRTL=true;
//			}
...
//				stBarUchur.Font = this.menuBar.Font;
		...
		void MainFormPaint(object sender, PaintEventArgs e)
		{
			...
			if(gImla.WordFinder==null || gImla.SpellCheker==null)
			{
...
//		}
		...
		void MenuTekrarClick(object sender, EventArgs e)
		{
			...
			if(menuImlaUEY.Checked){
				ngramtext = ngram.MakeNGram(gEditor,this.gUyghurcheSoz);
				MenuYengiClick(null,null);
				gEditor.Text = ngramtext;
				gEditor.RightToLeft = true;
			}
			else if(menuImlaULY.Checked){
				ngramtext = ngram.MakeNGram(gEditor,this.gLatincheSoz);
				MenuYengiClick(null,null);
				gEditor.Text = ngramtext;
				gEditor.RightToLeft = false;
			}
			else if(menuImlaULY.Checked){
				ngramtext = ngram.MakeNGram(gEditor,this.gSlawyancheSoz);
				MenuYengiClick(null,null);
				gEditor.Text = ngramtext;
				gEditor.RightToLeft = false;
			}
